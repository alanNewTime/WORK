...
function Pillars({ totalWidth }) {
  ...
  const firstPillarPosition = [-(totalWidth / 2) - 2, 3, 1]; // Shift slightly left from the window collection
  // The second pillar stays at the extreme right of the window bundle
  const secondPillarPosition = [totalWidth / 2 + 2, 6, 1]; // Shift slightly right from the window collection
    ...
    <>
      ...
      <group>
        <mesh position={firstPillarPosition}>
          <boxGeometry attach="geometry" args={[1, 12, 2]} />
      ...
      <group>
        <mesh position={secondPillarPosition}>
          <boxGeometry attach="geometry" args={[1, 18, 2]} />
          <meshStandardMaterial attach="material" color="brown" />
        </mesh>
        <mesh position={secondPillarPosition}>
          <boxGeometry attach="geometry" args={[1, 18, 2]} />
...
function Panel({ totalWidth }) {
  //console.log(totalWidth);

  // State to store top bar variables
  const [barRotation, setBarRotation] = useState(null);
  const [barLength, setBarLength] = useState(null);
  const [barTranslationY, setTranslationY] = useState(null);

  //brain behind the dealing with the top bar
  useEffect(() => {
    let newBarRotation;
    let newBarLength;
    let newBarTranslationY;
    // Check the totalWidth value and calculate top bar variables accordingly
    if (totalWidth === 0) {
      // 1 window
      newBarRotation = totalWidth - 2.1;
      newBarLength = 7.5;
      newBarTranslationY = 8.5;
    } else if (totalWidth === 3) {
      // 2 windows
      newBarRotation = totalWidth - 2.27;
      newBarLength = 9;
      newBarTranslationY = 8.5;
    } else if (totalWidth === 6) {
      // 3 windows
      newBarRotation = totalWidth - 2.2;
      newBarLength = 12;
      newBarTranslationY = 8.7;
    } else if (totalWidth === 9) {
      // 4 windows
      newBarRotation = totalWidth - 2.2;
      newBarLength = 14;
      newBarTranslationY = 8.7;
      ...
      // 5 windows
      newBarLength = 17;
      newBarTranslationY = 9;
      newBarRotation = totalWidth - 5.25;
    } else if (totalWidth === 15) {
      // 6 windows
      newBarRotation = totalWidth - 8.3;
      newBarLength = 20;
      newBarTranslationY = 9.5;
    }

    // Update the top bsr state with the new values
    setBarRotation(newBarRotation);
    setBarLength(newBarLength);
    setTranslationY(newBarTranslationY);
  }, [totalWidth]); // Add totalWidth as a dependency so the effect runs when it changes

  return (
    <>
      {/* top bar */}
      <mesh
        position={[0, barTranslationY, 0.95 - totalWidth / 80]}
        rotation={[Math.PI / 2, barRotation, 0]}
      >
        <boxGeometry attach="geometry" args={[barLength, 1.5, 1]} />
        <meshStandardMaterial attach="material" color="brown" />
      </mesh>

      {/* bottom bar */}
      <mesh position={[0, 5, 0.95 - totalWidth / 80]}>
        <boxGeometry attach="geometry" args={[totalWidth + 3, 1, 1.5]} />
        <meshStandardMaterial attach="material" color="brown" />
      </mesh>

      {/* pendinatura */}
      <mesh position={[-3, 7, 0.8]}>
        <boxGeometry attach="geometry" args={[1, 3, 1]} />
        <meshStandardMaterial attach="material" color="yellow" />
      </mesh>
      <mesh position={[4, 8, 0.8]}>
        <boxGeometry attach="geometry" args={[1, 6.5, 1]} />
        <meshStandardMaterial attach="material" color="yellow" />
      </mesh>
    </>
  );
...
function FoldableGlassPanel(props) {
...
export default FoldableGlassPanel;
